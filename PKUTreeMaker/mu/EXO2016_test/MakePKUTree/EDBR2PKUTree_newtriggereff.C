...
void EDBR2PKUTree::Loop(TString channelname, Double_t XS, Double_t totaleventnumber, Int_t IsData) {
	...
	TFile* pileupFile1 = TFile::Open("pileupDataRun2016B_69mb_80X.root");  
	...
	for (Long64_t jentry=0; jentry<nentries;jentry++) 
	{
		...
		if(IsData>0) {
			...
			if(npT < weights_pu2.size()){
                ...
                Double_t trigger_eff=1.0;
                if (ptlep1>50 && ptlep1<60)
                   {if (fabs(etalep1)<0.9){trigger_eff=0.975;}
                    else if (fabs(etalep1)<1.2){trigger_eff=0.921;}
                    else if (fabs(etalep1)<1.44){trigger_eff=0.921;}
                    else if (fabs(etalep1)<1.56){trigger_eff=0.868;}
                    else if (fabs(etalep1)<2.1){trigger_eff=0.87;}
                   }
                else if (ptlep1>60 && ptlep1<80)
                   {if (fabs(etalep1)<0.9){trigger_eff=0.972;}
                    else if (fabs(etalep1)<1.2){trigger_eff=0.936;}
                    else if (fabs(etalep1)<1.44){trigger_eff=0.905;}
                    else if (fabs(etalep1)<1.56){trigger_eff=0.843;}
                    else if (fabs(etalep1)<2.1){trigger_eff=0.881;}
                   }
                else if (ptlep1>80 && ptlep1<120)
                   {if (fabs(etalep1)<0.9){trigger_eff=0.966;}
                    else if (fabs(etalep1)<1.2){trigger_eff=0.902;}
                    else if (fabs(etalep1)<1.44){trigger_eff=0.894;}
                    else if (fabs(etalep1)<1.56){trigger_eff=0.692;}
                    else if (fabs(etalep1)<2.1){trigger_eff=0.872;}
                   }
                else if (ptlep1>120)
                   {if (fabs(etalep1)<0.9){trigger_eff=0.965;}
                    else if (fabs(etalep1)<1.2){trigger_eff=0.926;}
                    else if (fabs(etalep1)<1.44){trigger_eff=0.929;}
                    else if (fabs(etalep1)<1.56){trigger_eff=0.68;}
                    else if (fabs(etalep1)<2.1){trigger_eff=0.866;}
                   }
		...
		weight=lumiWeight*triggerWeight*eff_and_pu_Weight*nn*trigger_eff;
		...
		tmp_categoryID_channel=-1;// -1 for el; 1 for mu
			...
			if (isAnaHP>0 && ptlep1>55 && fabs(etalep1)<2.1){ npt_e = npt_e+1; } else{ isAnaHP=-1; }
			...
			//LP: 0.5<tau21<=0.75;
			if ( lep==11 && nLooseLep==1 && ptlep1>55 && fabs(etalep1)<2.1 && MET_et>80 && ptVlepJEC > 200.  && ptVhad>200 && fabs(yVhad)<2.4 && IDLoose>0 && num_bJet<1 && deltaRlepjet>pi_2 && fabs(delPhijetmet) >2.0 && fabs(delPhijetlep)>2.0 && tau21>0.45 && tau21<=0.75 && (( massVhadJEC >40 &&  massVhadJEC< 150 )) )
			{ isAnaLP=1.; } 
			else{ isAnaLP=-1.; }
			//NP: 0.75<tau21
			if ( lep==11 && nLooseLep==1 && ptlep1>55 && fabs(etalep1)<2.1 && MET_et>80 && ptVlepJEC > 200.  && ptVhad>200 && fabs(yVhad)<2.4 && IDLoose>0 && num_bJet<1 && deltaRlepjet>pi_2 && fabs(delPhijetmet) >2.0 && fabs(delPhijetlep)>2.0 && tau21>0.75 && (( massVhadJEC >40 &&  massVhadJEC< 150 )) )
			{ isAnaNP=1.; } 
			else{ isAnaNP=-1.; }


			//TTbar control
			if ( lep==11 && nLooseLep==1 && ptlep1>55 && fabs(etalep1)<2.1 && MET_et>80 && ptVlepJEC > 200. &&ptVhad>200 && fabs(yVhad)<2.4 && IDLoose>0  && num_bJet>0 &&  massVhadJEC>40 && massVhadJEC <150)
			{ isTTBarControl=1.; } 
