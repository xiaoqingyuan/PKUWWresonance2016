...
parser.add_option('-a', '--additioninformation',action="store",type="string",dest="additioninformation",default="EXO_50ns")
...
#parser.add_option('--category', action="store",type="string",dest="category",default="LP")
...
class doFit_wj_and_wlvj:
    ...
    def __init__(self, in_channel,in_signal_sample, in_mj_min=45, in_mj_max=140, label="", input_workspace=None):
        ...
        #rrv_mass_j.setRange("controlsample_fitting_range",45,130);
        rrv_mass_j.setRange("controlsample_fitting_range",40,130);

        ## directory where are the trees to be run
        #self.file_Directory="newTree-All/";
        #self.file_Directory="PrepareDataSet/";
        self.file_Directory="TTBarDataSet/";
        #self.file_Directory="output_Apr28_1/";
        self.signal_sample=in_signal_sample;
             

        ## taking the root file for data and mc
        ##self.file_data=("treeEDBR_pdata.root");
        ##self.file_pseudodata=("treeEDBR_pdata_reweight.root");
        ##self.file_signal=("treeEDBR_%s_xwh.root"%(self.signal_sample));
        ###WJets0 is the default PS model, WJets1 is the alternative PS model
        ##self.file_WJets0_mc = ("treeEDBR_WJetsPt180_xwh.root");
        ##self.file_VV_mc     = ("treeEDBR_VV_xwh.root");# WW+WZ
        ##self.file_TTbar_mc  = ("treeEDBR_TTBARpowheg_xwh.root");
        ##self.file_STop_mc   = ("treeEDBR_SingleTop_xwh.root");#single Top
        ...
        if self.wtagger_label=="HP" :
            if self.channel=="el":self.wtagger_cut=0.45 ; self.wtagger_cut_min=0. ;
            if self.channel=="mu":self.wtagger_cut=0.45 ; self.wtagger_cut_min=0. ;

        if self.wtagger_label=="HP" :
         if self.channel=="el":self.wtagger_cut=0.45 ; self.wtagger_cut_min=0. ;
         if self.channel=="mu":self.wtagger_cut=0.45 ; self.wtagger_cut_min=0. ;
         if self.wtagger_label=="LP": self.wtagger_cut=0.75 ; self.wtagger_cut_min=0.45 ;
         #if self.wtagger_label=="LP": self.wtagger_cut=0.75 ; self.wtagger_cut_min=0.45 ;
         if self.wtagger_label=="nocut": self.wtagger_cut=10000;

        self.categoryID=-1;
        if self.wtagger_label=="LP" and self.channel=="el": self.categoryID=-4
        if self.wtagger_label=="HP" and self.channel=="el": self.categoryID=-3
        if self.wtagger_label=="LP" and self.channel=="mu": self.categoryID= 4
        ...
        self.pfMET_cut = 40;#50; ## missing transverse energy
   ...
   #### Method to make a RooAbsPdf giving label, model name, spectrum, if it is mc or not and a constraint list for the parameters
    ...
    def make_Pdf(self, label, in_model_name, mass_spectrum="_mj", ConstraintsList=[]):
        ...
        if in_model_name == "2Gaus": ## double gaussian for the resonant of mJ
            ...
            rrv_mean1_gaus = RooRealVar("rrv_mean1_gaus"+label+"_"+self.channel,"rrv_mean1_gaus"+label+"_"+self.channel,83.5, 70., 86.5);
            rrv_sigma1_gaus = RooRealVar("rrv_sigma1_gaus"+label+"_"+self.channel,"rrv_sigma1_gaus"+label+"_"+self.channel,7.6,3.5,20.);
            gaus1 = RooGaussian("gaus1"+label+"_"+self.channel,"gaus1"+label+"_"+self.channel, rrv_x,rrv_mean1_gaus,rrv_sigma1_gaus);
        ...
        if in_model_name == "2_2Gaus":# 2Gaus + 2Gaus for the resonant W and Z part of the mJ spectra
            ...
            rrv_mean1_gaus = RooRealVar("rrv_mean1_gaus"+label+"_"+self.channel,"rrv_mean1_gaus"+label+"_"+self.channel,mean1_tmp, mean1_tmp-20, mean1_tmp+20);
            rrv_sigma1_gaus = RooRealVar("rrv_sigma1_gaus"+label+"_"+self.channel,"rrv_sigma1_gaus"+label+"_"+self.channel,sigma1_tmp, sigma1_tmp-30,sigma1_tmp+30 );
            gaus1 = RooGaussian("gaus1"+label+"_"+self.channel,"gaus1"+label+"_"+self.channel, rrv_x,rrv_mean1_gaus,rrv_sigma1_gaus);
        ...
        if in_model_name == "2Gaus_ttbar":
            ...
            mean1_tmp = 7.64813e+01; mean1_tmp_err = 1.30763e-01;
            deltamean_tmp = 2.0000e+00; deltamean_tmp_err = 1.24596e+00;
            sigma1_tmp = 7.16642e+00; sigma1_tmp_err = 1.46600e-01;
            scalesigma_tmp = 3.9142e+00; scalesigma_tmp_err = 9.91214e-01;
            frac_tmp = 6.23012e-01; frac_tmp_err = 1.22978e-02;
            #mean1_tmp = 7.6273e+01; mean1_tmp_err = 6.94e-01;
            #deltamean_tmp = 6.9129e+00; deltamean_tmp_err = 1.24e+00;
            #sigma1_tmp = 7.9599e+00; sigma1_tmp_err = 9.84e-01;
            #scalesigma_tmp = 3.6819e+00; scalesigma_tmp_err = 2.11e-01;
        ...
        if in_model_name == "GausChebychev_ttbar_failtau2tau1cut":
          ...
          p0_tmp = -4.75782e-01; p0_tmp_err = 3.22521e-02;
          p1_tmp = 6.64272e-02; p1_tmp_err = 4.21903e-02;
          frac_tmp = 2.84619e-01; frac_tmp_err = 2.47657e-02;
        ...
        if in_model_name == "ErfExp_ttbar":
            ...
            c0_tmp = -3.24407e-02;  c0_tmp_err = 3.46928e-03;
            offset_tmp = 7.85309e+01; offset_tmp_err = 3.87637e+00;
            width_tmp = 2.93778e+01; width_tmp_err = 1.57287e+00;
            #c0_tmp = -2.9893e-02 ; c0_tmp_err = 6.83e-03;
            #offset_tmp = 7.9350e+01 ; offset_tmp_err = 9.35e+00;
            #width_tmp = 3.3083e+01 ; width_tmp_err = 2.97e+00;
        ...
        if in_model_name == "ErfExp_ttbar_failtau2tau1cut":
            ...
            c0_tmp = -3.29392e-02; c0_tmp_err = 1.70632e-03;
            offset_tmp = 3.93490e+01; offset_tmp_err = 2.71901e+00;
            width_tmp = 3.04720e+01 ; width_tmp_err = 5.36037e+00;
...
## No Constraint
    ...
    ########## ---------------------------------------------------
    def get_mj_and_mlvj_dataset_TTbar_controlsample(self,in_file_name, label, jet_mass ="jet_mass_softdrop"):# jet_mass_softdrop jet_mass_pruned massVhad"):# to get the shape of m_lvj jet_mass="jet_mass_pr" , "ttb_ca8_mass_pr"
        ...
        for i in range(treeIn.GetEntries()):
            ...
            #        if treeIn.vTagID == 0: discriminantCut= 1;
            #        else: discriminantCut = -1;

            #    if (self.categoryID == 1 or self.categoryID == 3):# HP
            #        if treeIn.vTagID == 1: discriminantCut= 1;
            #        else: discriminantCut = -1;

            if TMath.Abs(treeIn.CategoryID) > 2:
                discriminantCut = -1;

                if (self.categoryID == -4 or self.categoryID == 4):# LP  #qun
                    #if TMath.Abs(treeIn.CategoryID) == 4: discriminantCut= 1;
                    if treeIn.vTagID == 0: discriminantCut= 1;
                    else: discriminantCut = -1;

                if (self.categoryID == -3 or self.categoryID == 3):# HP
                    if treeIn.vTagID == 1: discriminantCut= 1;
                    #if TMath.Abs(treeIn.CategoryID) == 3: discriminantCut= 1;
                    else: discriminantCut = -1;
            ...
            if discriminantCut >0 and treeIn.m_lvj> self.mass_lvj_min and treeIn.m_lvj< self.mass_lvj_max and tmp_jet_mass>rrv_mass_j.getMin() and tmp_jet_mass<rrv_mass_j.getMax() :
            ...
            if (discriminantCut >0 or discriminantCut <0) and treeIn.m_lvj> self.mass_lvj_min and treeIn.m_lvj< self.mass_lvj_max and tmp_jet_mass>rrv_mass_j.getMin() and tmp_jet_mass<rrv_mass_j.getMax() :
            ...
            if discriminantCut <0 and treeIn.m_lvj> self.mass_lvj_min and treeIn.m_lvj< self.mass_lvj_max and tmp_jet_mass>rrv_mass_j.getMin() and tmp_jet_mass<rrv_mass_j.getMax() :
                ...
                if treeIn.vTagID == -1:
                #if TMath.Abs(treeIn.CategoryID) == 5:
    ...
    def GetLumi(self):
        ...
        if self.channel=="el": return 0.209 
        if self.channel=="mu": return 0.209 
        #if self.channel=="el": return 3.0 
        #if self.channel=="mu": return 3.0 
